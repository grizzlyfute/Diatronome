...
import com.tuner.diatronome.Cores.MetronomeCore;
import com.tuner.diatronome.Cores.MetronomePlaybackService;
import com.tuner.diatronome.Cores.SettingsCore;
import com.tuner.diatronome.R;
import com.tuner.diatronome.Ui.MetronomeView;
import com.tuner.diatronome.Ui.NumberPickerDialog;
import com.tuner.diatronome.Ui.TimeSignatureDialog;
import com.tuner.diatronome.databinding.FragmentMetronomeBinding;

public class MetronomeFragment extends Fragment
{
  private NumberPickerDialog m_bpmDialog;
  private final TimeSignatureDialog m_timeSignatureDialog;

  View m_root;
  MetronomeView m_metronomeView;
  final private String TAG_DIALOG = "dialog";
                           ...
                           Bundle savedInstanceState)
  {
    ...
    m_bpmDialog = new NumberPickerDialog();
    m_bpmDialog.setMin(MetronomeCore.BPM_MIN);
    m_bpmDialog.setMax(MetronomeCore.BPM_MAX);
    m_bpmDialog.setTitle(R.string.metronome_bpm);
    m_bpmDialog.setOnValidateListener(new NumberPickerDialog.NumberPickerDialogListener()
    {
      @Override
    ...
    Button btnBpm = m_root.findViewById(R.id.btnMetronomeTempo);
    if (btnBpm != null)
    {
      btnBpm.setOnClickListener(new View.OnClickListener()
      {
        ...
        public void onClick(View v)
        {
          m_bpmDialog.setValue(MetronomeCore.getInstance().getTempoBpm());
          m_bpmDialog.show(getChildFragmentManager(), TAG_DIALOG);
        }
      });
    }
    ...
    if (MetronomeCore.getInstance().getIsPlaying())
    {
      setTick((MetronomeCore.getInstance().getCurrentTick() - 1 + MetronomeCore.getInstance().getDivision()) % MetronomeCore.getInstance().getDivision());
    }
  ...
  public void setPlay(boolean isPlaying)
  {
    ...
    if (btnPlay != null)
    {
      ...
      else
      {
        ...
        m_metronomeView.setReverse(false);
  ...
  public void setTempo(int bpm)
  {
    ...
    TextView btnTempo = m_root.findViewById(R.id.btnMetronomeTempo);
    if (btnTempo != null)
    {
      btnTempo.setText ("" + bpm + " bpm");
    }
    SeekBar skbTempo = m_root.findViewById(R.id.skbMetronomeTempo);
    if (skbTempo != null)
    {
  ...
  public void setTick(int tickNb)
  {
    ...
    if (tickNb >= 0)
    {
      ...
      {
        if (MetronomeCore.getInstance().getBidirectionalNeedle())
        {
          m_metronomeView.setReverse(!m_metronomeView.getReverse());
        }
        else
        {
          // skip reverse animated
          m_metronomeView.setAngle(-180.0f, 0);
        }
      }
      m_metronomeView.setAngle(-180.0f * (tickMax - tickNb - 1) / tickMax, periodMs);
    }
    else
    {
      m_metronomeView.setReverse(false);
      m_metronomeView.setAngle(-180, 0);
    }
