  ...
  extends BaseCore
{
  ...
  private MetronomeCore()
  {
    ...
    m_mutexTick = new ReentrantLock(true);
  ...
  public void setBidirectionalNeedle(boolean isBidirectional)
  {
    m_isBidirectionalNeedle = isBidirectional;
  }
  public boolean getBidirectionalNeedle()
  {
  ...
  private void tick()
  {
    if (m_currentTick < 0) return;
    try
    {
      try
      {
        if (!m_mutexTick.tryLock(getPeriodMs(), TimeUnit.MILLISECONDS)) return;
      }
