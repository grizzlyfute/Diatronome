  ...
  extends BaseCore
{
  ...
  public static byte[] toAudioBytes(final short[] pcm)
  {
    ...
    byte audioByte[] = new byte[pcm.length * 2 * (AUDIO_FORMAT == AudioFormat.CHANNEL_OUT_STEREO ? 2 : 1)];
    int index_byte = 0;
  ...
  public static short[] generatePcm(double frequency, double durationMs, int waveForm)
  {
    ...
    // Generate sample
    // n = dur * ABR / 1000
    int numSample = (int)(durationMs * AUDIO_BIT_RATE / 1000.0);
    // The duration become inaccurate. This allow to have an integer number of periods, and avoid audio glitch
    numSample = (int)(Math.ceil(Math.ceil (numSample * (frequency / AUDIO_BIT_RATE)) / (frequency / AUDIO_BIT_RATE)));
    ...
    switch (waveForm)
    {
      case WAVEFORM_SINE:
        ...
        {
          soundPcm[i] = (short)(range * (Math.sin (2 * Math.PI * i * frequency / AUDIO_BIT_RATE) + 1)/2 + min);
        }
        break;
  ...
  {
    final int filterDurationInSamples = Math.min(filterDurationInMs * AUDIO_BIT_RATE / 1000, audio.length / 2);
  ...
  public void startPlaying(int octave, int note)
  {
    final double frequency = getFrequency(octave, note);
    final int frameSize = getAudioFrameSize();
    final short[] pcm = generatePcm(frequency, 1000.0, m_waveForm);
    ...
    byte[] audioPcm = new byte[frameSize * 2 * (pcmStart.length + pcm.length + pcmEnd.length)];
