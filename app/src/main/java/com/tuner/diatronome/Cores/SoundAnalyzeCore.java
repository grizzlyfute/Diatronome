...
import com.tuner.diatronome.Cores.SoundAnalyzer.ISoundAnalyzer;
import com.tuner.diatronome.Cores.SoundAnalyzer.SoundAnalyzerEnvelop;
import com.tuner.diatronome.Cores.SoundAnalyzer.SoundAnalyzerFourier;
import com.tuner.diatronome.Cores.SoundAnalyzer.SoundAnalyzerPeakDetector;
import com.tuner.diatronome.Cores.SoundAnalyzer.SoundAnalyzerYin;
  ...
  implements Runnable
{
  ...
  public void run()
  {
    ...
    while (!Thread.interrupted())
    {
      ...
      if (lenByteToRead > 0 || timeLoopRoundMs > 0)
      {
        long diff = System.currentTimeMillis() - tsLastLoop;
        tsLastLoop += diff;
        diff = (long)Math.max (1000 * lenByteToRead / AUDIO_SAMPLE_RATE,
                               timeLoopRoundMs) - diff;
        if (diff > 0)
        {
          try
          {
      ...
      if (offsetBuffer >= readBuffer.length)
      {
        ...
        if (intensity > thresholdIntensity)
        {
          ...
          doMessage (frequency / freqSmooth.length, intensity / freqSmooth.length);
          timeLoopRoundMs = 300;
        }
        else
        {
