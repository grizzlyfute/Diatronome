  ...
  extends BaseCore
{
  ...
  private byte[] generatePcmWithFadeInOut(ASoundGenerator generator, float pitch)
  {
    ...
    if (pitch > 0)
    {
      int durationMs = AUDIO_DURATION_MS;
  ...
  private void tick()
  {
    ...
    try
    {
      ...
      writeAudioTrack(waveCurrent, Math.min (waveCurrent.length, AudioUtils.getAudioByteLen(durationMs)), durationMs);
      ...
      writeAudioTrack(m_waveSilence, remainingBytes, durationMs);
